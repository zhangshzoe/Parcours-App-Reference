...
assert.equal(lessons.length,450);
assert.equal(new Set(lessons.map(l=>l.id)).size,450,'Duplicate lesson IDs');
assert.equal(allWords.length,1350);
assert.equal(new Set(allWords.map(w=>w.id)).size,1350,'Duplicate vocabulary IDs');
for(const [level,count] of Object.entries(expected))assert.equal(lessons.filter(l=>l.language==='fr'&&l.level===level).length,count,'fr '+level);
assert.equal(lessons.filter(l=>l.language==='en'&&l.level==='A1').length,50,'en A1');
assert.equal(lessons.filter(l=>l.language==='de'&&l.level==='A1').length,50,'de A1');
...
for(const level of Object.keys(expected))assert.equal(new Set(lessons.filter(l=>l.language==='fr'&&l.level===level).map(l=>l.title)).size,50,'fr '+level+' duplicate titles');
for(const language of ['en','de'])assert.equal(new Set(lessons.filter(l=>l.language===language).map(l=>l.title)).size,50,language+' duplicate titles');
const errors=[];
...
console.log('PASS: 450 unique lessons across French, English and German, 1350 vocabulary cards, bilingual dialogue pairs, answer indices and reference writing lengths.');
